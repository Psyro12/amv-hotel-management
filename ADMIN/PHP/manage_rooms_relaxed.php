...
try {
    ...
    if ($action === 'add') {
        ...
        $type = $bed_type; 
        $stmt = $conn->prepare("INSERT INTO rooms (name, type, price, capacity, size, bed_type, description, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, 1)");
        $stmt->bind_param("ssdisss", $name, $type, $price, $capacity, $size, $bed_type, $description);
    ...
    elseif ($action === 'edit') {
        ...
        $stmt = $conn->prepare("UPDATE rooms SET name=?, price=?, capacity=?, size=?, bed_type=?, description=? WHERE id=?");
        $stmt->bind_param("sdisssi", $name, $price, $capacity, $size, $bed_type, $description, $id);
