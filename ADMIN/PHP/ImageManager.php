...
class ImageManager
{
    ...
    public function uploadImage($uploadedFile, $userId = null, $roomName = null)
    {
        try {
            // Validate the file
    ...
    private function storeImageMetadata($filename, $filePath, $originalName, $mimeType, $fileSize, $userId, $roomName = null)
    {
        $sql = "INSERT INTO images (filename, file_path, original_name, mime_type, file_size, created_by, image_name) 
                VALUES (?, ?, ?, ?, ?, ?, ?)";

        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$filename, $filePath, $originalName, $mimeType, $fileSize, $userId, $roomName]);
