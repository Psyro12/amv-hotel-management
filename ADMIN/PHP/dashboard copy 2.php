...
// =================================================================
// 2. FETCH DASHBOARD STATS (UPDATED FOR OVERVIEW)
// =================================================================

// A. Active Bookings (Guests In-House + Arriving Today)
...
// B. Monthly Revenue (Confirmed bookings in Current Month)
$sql_m_rev = "SELECT SUM(total_price) as total FROM bookings 
              WHERE status = 'confirmed' 
              AND MONTH(check_in) = MONTH(CURDATE()) 
              AND YEAR(check_in) = YEAR(CURDATE())";
$res_m_rev = $conn->query($sql_m_rev);
$monthlyRevenue = $res_m_rev->fetch_assoc()['total'] ?? 0;

// C. Occupancy Rate Calculation

// 1. Get Total Active Rooms
...
// 2. Get Count of Occupied Rooms (In-House)
// 🔴 FIX: Added 'DISTINCT' to prevent double-counting the same room
$sql_occ = "SELECT COUNT(DISTINCT br.room_id) as total 
...
$occupiedRooms = $res_occ->fetch_assoc()['total'] ?? 0;
...
$sql_pending = "SELECT COUNT(*) as total FROM bookings 
                WHERE status = 'confirmed' 
                AND check_in = '$today_date' 
                AND arrival_status != 'in_house' 
                AND arrival_status != 'checked_out'";

$res_pending = $conn->query($sql_pending);
$pendingRequests = $res_pending->fetch_assoc()['total'] ?? 0;

// E. Total Orders (Active Kitchen Queue)
// Counts orders that are waiting or being prepared
...
// =================================================================
// 3. FETCH CHART DATA (With 2PM / 12NN Logic)
// =================================================================
$pieStats = [
    'complete' => 0,
    'noshow' => 0,
...
$sql_charts = "SELECT status, arrival_status, check_in, check_out, total_price, MONTH(check_in) as month, YEAR(check_in) as year 
               FROM bookings 
               WHERE YEAR(check_in) = '$curY'";

$result_charts = $conn->query($sql_charts);

while ($row = $result_charts->fetch_assoc()) {
    $checkInTime = new DateTime($row['check_in'] . ' 14:00:00'); // 2:00 PM
    $checkOutTime = new DateTime($row['check_out'] . ' 12:00:00'); // 12:00 NN

    // =========================================================
    // 1. PIE CHART LOGIC (Filtered to CURRENT MONTH Only)
    // =========================================================
    // Check if the booking month equals the current server month (1-12)
    // AND if the booking year equals the current server year
    if ($row['month'] == date('n') && $row['year'] == date('Y')) {
        ...
        // 🔴 FIX: Catch explicit 'no_show' status from database
        elseif ($row['arrival_status'] === 'no_show') {
            $pieStats['noshow']++;
        }
        // 🔴 FIX: Catch implicit/dynamic no-shows (Confirmed but late & status empty)
    ...
    // =========================================================
    // 2. BAR CHART LOGIC (Filtered to CURRENT YEAR)
    // =========================================================
    // Revenue chart needs data for the whole year, not just this month
    if ($row['status'] === 'confirmed' && $row['year'] == $currentYear) {
        $month = (int) $row['month'];
...
$sql_rooms_fetch = "SELECT * FROM rooms ORDER BY is_active DESC, id ASC";
...
<head>
    ...
    <style>
        ...
        .booking-table-container {
            ...
            /* --- NEW SCROLLING PROPERTIES --- */
            height: 75vh;
            /* Adjust this value to fit your screen preferences */
            overflow-y: auto;
            /* Enables vertical scrolling */
            /* Keeps horizontal scrolling for mobile */
            /* Needed for sticky context */
        }
...
<body>
            ...
            <div class="page-content">
                <div class="page" id="dashboard" style="padding: 20px">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <div
                            style="display: flex; align-items: center; gap: 10px; background: #fff; padding: 5px 10px; border-radius: 8px; border: 1px solid #ddd;">
                            <label style="font-size: 0.85rem; font-weight: 600; color: #555;">View Month:</label>
                            <input type="month" id="dashboardMonthPicker" onchange="fetchDashboardCards()"
                                value="<?php echo date('Y-m'); ?>"
                                style="border: none; outline: none; font-family: 'Montserrat', sans-serif; color: #333; font-weight: 600; cursor: pointer;"
                                onchange="fetchDashboardCards()">
                        </div>
                    </div>
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="stat-card">
                            <h3 class="stat-value"><span
                                    id="stat_revenue">₱<?php echo number_format($monthlyRevenue, 0); ?></span></h3>
                        ...
                        <div class="stat-card">
                            ...
                            <p class="stat-label">Pending Arrivals</p>
                        </div>

                        <div class="stat-card">
                            <h3 class="stat-value" id="stat_orders"><?php echo $totalOrders; ?></h3>
                            <p class="stat-label">Kitchen Orders</p>
                        </div>
                    </div>
                ...
                <!-- Guest Page -->
                <div class="page" id="guests" style="overflow: hidden;">
                ...
                <!-- Booking Page -->
                <div class="page" id="bookings" style="overflow: hidden;">
                    <div class="p-3" style="display: flex; flex-direction: column; height: 100%;">
                                                ...
                                                <td>
                                                    <button class="btn-secondary" style="padding:5px 10px; font-size:0.8rem;"
                                                        onclick="openBookingAction(
                                                         ...
                                                         '<?php echo $row['booking_source']; ?>' /* <--- NEW ARGUMENT ADDED HERE */
                                                        )">
                                                        View
                                                    </button>
                                                </td>
                ...
                <div class="page" id="food-ordered" style="overflow: hidden;">
                    <div class="p-3" style="display: flex; flex-direction: column; height: 100%;">
                ...
                <div class="page" id="transactions" style="overflow: hidden;">
                    <div class="p-3" style="display: flex; flex-direction: column; height: 100%;">
                ...
                <!-- Settings Page -->
                <div class="page" id="settings" style="overflow: hidden;">
                    <div class="p-3">
                ...
                <!-- Hotel Events View -->
                <div id="view-news" class="settings-view px-3 mb-3" style="overflow: hidden;">
                    <div class="settings-header" style="justify-content: space-between;">
                        <div style="display:flex; align-items:center; gap:15px;">
                            <button class="back-btn-settings" onclick="openSettingsView('settings-home')">⬅
                                Back</button>
                ...
                <div id="view-food-menu" class="settings-view px-3 mb-3" style="overflow: hidden;">
                ...
                <!-- Hotel Events View -->
                <div id="view-events" class="settings-view px-3 mb-3" style="overflow: hidden;">
                    <div class="settings-header" style="justify-content: space-between;">
                        <div style="display:flex; align-items:center; gap:15px;">
                            <button class="back-btn-settings" onclick="openSettingsView('settings-home')">⬅
                                Back</button>
                ...
                <div id="view-receipts" class="settings-view px-3 mb-3" style="overflow: hidden;">
                    <div class="settings-header" style="justify-content: space-between;">
                        <div style="display:flex; align-items:center; gap:15px;">
                            <button class="back-btn-settings" onclick="openSettingsView('settings-home')">⬅
                                Back</button>
                    ...
                    <div class="receipt-gallery-container" style="height: 75vh; overflow-y: auto; padding: 5px;">
            ...
            <div class="ab-modal-body" style="padding: 25px;">
                <form id="amenityForm">
                    ...
                    <div class="ab-mb-3">
                        ...
                        <small style="color: #888;">Use FontAwesome 5 classes (e.g. fas fa-wifi)</small>
                    </div>
            ...
            <div class="ab-modal-body" style="padding: 25px;">
                ...
                <div class="ab-grid-2 ab-mb-3">
                    <div>
                        ...
                        <input type="text" id="resched_checkin" class="custom-date-input" placeholder="Select Date"
                            readonly>
                    </div>
                    <div>
                        <label class="ab-label">New Check-Out</label>
                        <input type="text" id="resched_checkout" class="custom-date-input" placeholder="Select Date"
                            readonly>
                    </div>
                </div>
                ...
                <div class="ab-grid-footer" style="margin-top: 15px;">
                    ...
                    <button class="ab-submit-btn" onclick="submitReschedule()">Confirm Change</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
            ...
            new Chart(newCanvas.getContext('2d'), {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        data: dataValues, // 12 values from PHP
                        backgroundColor: '#B88E2F',
                        borderRadius: 4,
                        barPercentage: 0.6
                    }]
        ...
        // --- 5. FETCH CARDS (Keep existing logic for top cards) ---

        function fetchDashboardCards() {
            const picker = document.getElementById('dashboardMonthPicker');
            const selectedMonth = picker ? picker.value : new Date().toISOString().slice(0, 7);

            fetch(`get_dashboard_stats.php?date=${selectedMonth}&_t=${new Date().getTime()}`)
                .then(res => res.json())
                .then(data => {
                    if (data.status === 'success') {
                        // 1. Update Cards
                        ...
                        // 3. Calculate fresh percentages from the NEW data
                        ...
                        // 4. Update Progress Bar Text (FIXED IDs)
                        ...
                        // 5. Update Progress Bar Widths (FIXED IDs)
                        ...
                        // 6. Also update the text values inside the stat cards
                        document.getElementById('stat_pending').innerHTML = data.pending;
                        document.getElementById('stat_orders').innerHTML = data.kitchen_orders; // Or use data from API if available
                    }
                })
        ...
        function openBookingAction(id, name, ref, rooms, checkin, checkout, price, arrivalStatus, amountPaid, currentLabel, createdAt, bookingSource) {
            ...
            // Scenario C: Pending Reservation
                ...
                if (checkin > todayStr) {
                    ...
                    // --- NEW RESCHEDULE BUTTON (With 3-Day Rule Check) ---
        ...
        function openEditRoomModal(id, name, price, type) {
            document.getElementById('roomId').value = id;
            document.getElementById('roomNameInput').value = name;
            document.getElementById('roomPriceInput').value = price;
            document.getElementById('roomTypeInput').value = type; // Assuming type matches option values
        ...
        function openEditRoomModal(id, name, price, bedType, capacity, size, description, filePath, isBooked, amenities) {
            ...
            // 🟢 FIX: LOCK THE CUSTOM "TYPE" SELECT MANUALLY
    ...
    <div class="dropdown-item-row" style="${bgStyle}" 
        ...
        function openExtendModal(id, currentCheckout) {
            ...
            // 2. Initialize Flatpickr specifically for this modal
            ...
            extendPickerInstance = flatpickr("#extend_date_picker", {
                ...
                // 1. Enable Fly-Over Mode
                static: false,
                appendTo: document.body, // Attaches to body so it floats over everything

                // 2. Apply the "Compact" class to fix the width issue
                onReady: function (selectedDates, dateStr, instance) {
                    instance.calendarContainer.classList.add("compact-theme");
                    // Remove the wide-calendar class if it was accidentally applied
                    instance.calendarContainer.classList.remove("wide-news-calendar");
                }
            });
    ...
    `;
        ...
        function openRescheduleModal(reference) {
            ...
            reschedCheckinFP = flatpickr("#resched_checkin", {
                ...
                // --- ADD THIS BLOCK ---
                onReady: function (selectedDates, dateStr, instance) {
                    // Force COMPACT class for this specific calendar
                    instance.calendarContainer.classList.add("compact-theme");
            ...
            fetch('guest_reschedule.php', {
                ...
                .then(data => {
                    if (data.status === 'success') {
                        ...
                        renderRescheduleAlternatives(data.message, data.next_date, data.alternatives);
                    }
                    else {
                        alert("❌ " + data.message);
                        btn.innerText = originalText;
        ...
        function renderRescheduleAlternatives(conflictMsg, nextDate, rooms) {
            ...
            if (rooms.length > 0) {
                rooms.forEach(room => {
                    ...
                    roomsHtml += `
            <div class="room-card" onclick="selectAltRoom(this, ${room.id})" style="border:1px solid #ddd; margin-bottom:0;">
                <div class="room-card-check"></div>
            ...
            modalBody.innerHTML = `
        <div style="background-color: #FEE2E2; color: #B91C1C; padding: 12px; border-radius: 6px; font-size: 0.9rem; margin-bottom: 15px; border: 1px solid #FECACA;">
            <i class="fas fa-times-circle"></i> <strong>Room Unavailable</strong><br>
            ${conflictMsg}
            ${nextDate ? `<br><small>Becomes free on: <strong>${nextDate}</strong></small>` : ''}
        </div>

        <h4 style="margin:0 0 10px 0; font-size:1rem; color:#333;">Select an Alternative Room:</h4>
        
        <div class="room-selection-grid" style="grid-template-columns: 1fr 1fr; gap: 10px; max-height: 300px; overflow-y: auto; padding: 2px;">
            ${roomsHtml}
        </div>

        <div class="ab-grid-footer" style="margin-top: 15px;">
            <button class="btn-secondary" onclick="location.reload()">Cancel</button>
            <button class="ab-submit-btn" id="btn-confirm-alt" onclick="confirmAltRoom()" disabled style="opacity:0.5;">
                Confirm New Room
            </button>
        </div>
    `;
            ...
            fetch('get_payment_settings.php')
                ...
                .then(data => {
                    if (data.status === 'success') {
                        ...
                        editQrPlace.style.display = 'block';
