<?php
// FILE: API/api_get_notifications.php
...
// 🟢 FETCH LOGIC: Match by Email AND Source
// This ensures "John (Facebook)" doesn't see "John (Google)" notifications.
...
if ($stmt) {
    ...
    if ($stmt->execute()) {
        ...
        error_log("Database Execute Error (Notifications): " . $stmt->error);
        echo json_encode(['success' => false, 'message' => 'System error']);
    }
    $stmt->close();
} else {
    error_log("Database Prepare Error (Notifications): " . $conn->error);
    echo json_encode(['success' => false, 'message' => 'System error']);
}
