...
// We check if the query succeeds to prevent crashes on empty DBs
...
// This ensures that even if you don't have an icon column in DB, the UI looks good.
function getIconForTitle($title)
{
    $t = strtolower(strip_tags($title));
    if (strpos($t, 'check-in') !== false)
        return 'fa-regular fa-clock';
    if (strpos($t, 'reservation') !== false || strpos($t, 'payment') !== false)
        return 'fa-solid fa-credit-card';
    if (strpos($t, 'no-show') !== false)
        return 'fa-solid fa-user-slash';
    if (strpos($t, 'extension') !== false)
        return 'fa-solid fa-calendar-plus';
    if (strpos($t, 'cancellation') !== false)
...
// --- 7. Fetch Policies from Database ---
$policies = [];

// Secure Query: Selects all terms. We use error suppression (@) in case table structure varies slightly.
$sql_terms = "SELECT * FROM terms_conditions";
...
if ($result_terms && mysqli_num_rows($result_terms) > 0) {
    while ($row = mysqli_fetch_assoc($result_terms)) {
        ...
        // Dynamic Icon Logic: Use DB icon if exists, else guess it
        $icon = (isset($row['icon_class']) && !empty($row['icon_class']))
            ? $row['icon_class']
            : getIconForTitle($row['title']);
    ...
    $policies[] = [
        ...
        'content' => '<p>Policies are currently being updated by the administrator.</p>',
        'icon_class' => 'fa-solid fa-info-circle'
    ];
}

// --- 8. Fetch "Last Updated" Date ---
// This will work even if you haven't added the updated_at column yet (it falls back safely).
$last_updated_date = date('F Y'); // Default to today
$date_sql = "SELECT updated_at FROM terms_conditions ORDER BY updated_at DESC LIMIT 1";
$date_res = @mysqli_query($conn, $date_sql); // @ suppresses error if column doesn't exist
...
<head>
    ...
    <title>Hotel Policies - AMV Hotel</title>
    ...
    <style>
        ...
        * {
        ...
        .policy-content li strong::before {
            content: '\f05a';
    ...
    <div class="page-content-wrapper">
        ...
        <div class="page-header">
            <div class="header-content">
                <h1 class="page-title">Hotel Policies</h1>
                <p class="page-subtitle">Guidelines to ensure a pleasant stay for everyone.</p>
            ...
            <div class="policy-container">
                ...
                if (!empty($policies)) {
                    foreach ($policies as $policy) {
                        $iconClass = htmlspecialchars($policy['icon_class']);
                    ...
                    echo '<p style="text-align:center; padding:30px;">No policies found.</p>';
                }
                ?>
            ...
            <div class="footer-grid-container">
                ...
                <div class="footer-contact-list">
                    <a href="tel:<?php echo $hotel_phone_link; ?>" class="footer-contact-item"><i
                            class="fas fa-phone-alt"></i> <span><?php echo $hotel_phone; ?></span></a>
                    <a href="mailto:<?php echo $hotel_email; ?>" class="footer-contact-item"><i
                            class="fas fa-envelope"></i> <span><?php echo $hotel_email; ?></span></a>
