...
// Static query, safe to use standard execution
...
<head>
    ...
    <title>Events Gallery - AMV Hotel</title>
    ...
    <style>
        ...
        * {
        ...
        .page-header {
            ...
            height: 60vh;
            min-height: 400px;
            background-image: url('../../IMG/hotel_events.png');
        ...
        @media (max-width: 576px) {
            ...
            /* Maintain bento spans in 2-column mobile layout */
            ...
            /* Hide description for small 1x1 items to avoid clutter */
            .span-1-1 .bento-desc {
                display: none;
            }
        ...
        /* --- FIXED FOOTER STYLES (MATCHING HOME) --- */
...
<body>
    ...
    <div class="page-content-wrapper">
        ...
        <div class="page-header">
            <div class="header-content">
                <h1 class="page-title">EVENTS GALLERY</h1>
                <p class="page-subtitle">Explore a collection of our most memorable moments, grand celebrations, and
        ...
        <div class="gallery-container">
            <div class="bento-grid">
                ...
                // We select ALL events here, ordered by date descending
                $query = "SELECT * FROM hotel_events ORDER BY event_date DESC";
                $result = mysqli_query($conn, $query);

                // 2. Define the Layout Pattern (repeating array of classes)
                $layoutPattern = [
                    'span-2-2', // 1. Big Square
                    'span-1-1', // 2. Small
                    'span-1-2', // 3. Tall
                    'span-1-1', // 4. Small
                    'span-2-1', // 5. Wide
                ...
                $patternCount = count($layoutPattern);

                if ($result && mysqli_num_rows($result) > 0) {
                    $i = 0;
                    while ($evt = mysqli_fetch_assoc($result)) {

                        // 3. Assign Class based on Index Loop
                        $gridClass = $layoutPattern[$i % $patternCount];

                        // 4. Handle Image Path
                        $imgBase = '../../room_includes/uploads/events/';
                        $imgUrl = !empty($evt['image_path'])
                            ? $imgBase . htmlspecialchars($evt['image_path'])
                            : $placeholder;

                        // 5. Format Date
                        $dateObj = new DateTime($evt['event_date']);
                        $dateStr = $dateObj->format('M Y'); // e.g. "Dec 2024"
                    ...
                    // Empty State
                    echo '<div style="grid-column: 1 / -1; text-align: center; padding: 50px; color: #888;">No events found in the gallery.</div>';
                }
                ?>
            </div>
        </div>
    ...
    <?php if (isset($_GET['msg_success']) && $_GET['msg_success'] == 1): ?>
        <script>window.onload = function () { showCustomAlert('Success', 'Message successfully sent to Admin!', 'fa-check-circle'); }</script>
    <?php endif; ?>
