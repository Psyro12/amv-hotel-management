...
if ($roomParam !== '') {
    // 🟢 UPDATED: Query the 'rooms' table instead of 'room_image_details'
    $stmt = mysqli_prepare($conn, "SELECT id, name, description, image_path, price, capacity, size, bed_type, amenities FROM rooms WHERE name = ? LIMIT 1");
    if ($stmt) {
...
$imageUrl = '../../room_includes/uploads/images/' . trim($rawPath);
$roomId = $room['id'];
$roomPricePerNight = (float)$room['price'];
?>
                ...
                <div class="room-details">
                    ...
                    <div class="p-3">
                        ...
                        <div style="display: flex; gap: 15px; margin-bottom: 15px;">
                            <span style="font-size: 13px; color: #b8860b; font-weight: 700; background: #fdf5e6; padding: 4px 12px; border-radius: 6px; display: inline-flex; align-items: center; gap: 6px;">
                                <i class="fa-solid fa-user-group"></i> <?php echo h($room['capacity']); ?> GUESTS
                            </span>
                            <span style="font-size: 13px; color: #b8860b; font-weight: 700; background: #fdf5e6; padding: 4px 12px; border-radius: 6px; display: inline-flex; align-items: center; gap: 6px;">
                                <i class="fa-solid fa-ruler-combined"></i> <?php echo h($room['size']); ?>
                            </span>
                            <span style="font-size: 13px; color: #b8860b; font-weight: 700; background: #fdf5e6; padding: 4px 12px; border-radius: 6px; display: inline-flex; align-items: center; gap: 6px;">
                                <i class="fa-solid fa-bed"></i> <?php echo h($room['bed_type']); ?>
                            </span>
                        </div>
