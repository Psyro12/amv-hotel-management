<?php
// USER/PHP/news_details.php

// --- 1. Security Headers & Session Security ---
...
// --- 5. Get News Details Logic (SECURED) ---
$id = isset($_GET['id']) ? intval($_GET['id']) : 0;

// Use Prepared Statement for fetching news (Protects against SQL Injection)
$stmt_news = $conn->prepare("SELECT * FROM hotel_news WHERE id = ?");
...
$dateObj = new DateTime($news['news_date']);
$formattedDate = $dateObj->format('F d, Y');
$imgBase = '../../room_includes/uploads/news/';

// Handle image path (taking the first image if multiple are stored)
$rawPath = explode(',', $news['image_path'])[0];
$cleanPath = htmlspecialchars(trim($rawPath)); // Sanitize path
$imgUrl = !empty($cleanPath) ? $imgBase . $cleanPath : "https://via.placeholder.com/1920x800?text=No+Image";
?>
...
<head>
    ...
    <title><?php echo htmlspecialchars($news['title']); ?> - AMV Hotel</title>
    ...
    <style>
        ...
        * {
        ...
        /* --- 2. HERO SECTION (Fixed Background - Matches Event Gallery) --- */
        .news-hero {
            position: relative;
            width: 100%;
            height: 60vh;
...
<body>
    ...
    <div class="page-content-wrapper">
        ...
        <div class="news-hero" style="background-image: url('<?php echo $imgUrl; ?>');">
            <div class="hero-overlay"></div>
            <div class="hero-content">
                <span class="hero-label">News & Updates</span>
                <h1 class="hero-title"><?php echo htmlspecialchars($news['title']); ?></h1>
            </div>
        </div>

        <div class="content-container">
            <h2 class="news-body-title"><?php echo htmlspecialchars($news['title']); ?></h2>
            <div class="divider"></div>

            <div class="news-text">
                <p
                    style="text-align:center; color:#999; text-transform:uppercase; font-size:0.85rem; letter-spacing:1px; margin-bottom:30px;">
                    Published on <?php echo $formattedDate; ?>
                </p>
